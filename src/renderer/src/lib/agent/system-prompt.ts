...
function buildKnowledgeBaseReminder(): string | null {
  ...
  return [
    ...
    '',
    '流程（严格按顺序执行，不可跳过任何步骤）：',
    '1. 第一步（必做）：如果用户未明确指定知识库名称，必须先调用 get_my_kb_list 获取列表，然后向用户展示选项并等待用户选择。绝不能跳过此步。',
    '2. 第二步（必做）：用户选定知识库后，如果用户未明确指定目录名称，必须调用 get_kb_tree 获取该知识库的目录树，然后向用户展示目录选项并等待用户选择。绝不能跳过此步直接保存。',
    '3. 第三步：用户已明确指定知识库和目录（或已在前面步骤中完成选择），此时才调用 save_chat_to_kb，并将 parentId 设为用户所选目录的 ID。',
    '4. 缺少参数时优先调用工具查询，再向用户询问，不要凭空猜测知识库名称或目录名称。',
    '5. 即使只有根目录一个选项，也要向用户确认后再保存，不得自行决定保存到根目录。',
    '',
